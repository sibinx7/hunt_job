...
window.ParsleyValidator.addValidator('gegroup',function(value,requirement){
    // for each and find total value
        var totalSum = parseFloat(0)
        $(requirement).each(function(index,value){
            totalSum = parseFloat(totalSum) + parseFloat($(this).val())
        })
        console.log(value+","+totalSum)
        return parseFloat(value) >= parseFloat(totalSum)
    },32)
    .addMessage('en','This value should be greater')
